...
Local AI Video Dubbing Studio is a Windows-first, **local-first** application for preparing dubbed versions of media with AI. This phase provides fully local speech-to-text: uploaded audio or video is transcribed on your machine, with no paid cloud API or remote transcription service.
...
- Translation, TTS, voice cloning, dubbing, synchronization, and final video rendering are intentionally not implemented in this phase.

## Project layout

- `src/local_dubbing/stt/` — modular STT data models, formatter, engine boundary, and faster-whisper adapter.
- `app.py` — Streamlit speech-to-text user interface.
- `tests/` — fast unit tests that do not download a model.
...
2. Add optional offline translation adapters.
